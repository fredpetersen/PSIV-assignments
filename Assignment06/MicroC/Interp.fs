...
and eval e locEnv gloEnv store : int * store =
  match e with
    ...
    | PreInc acc -> let (loc, store1) = access acc locEnv gloEnv store (* CHANGED FOR ASSIGNMENT 6 *)
                    let res = (getSto store1 loc) + 1                  (* CHANGED FOR ASSIGNMENT 6 *)
                    (res, setSto store1 loc res)                       (* CHANGED FOR ASSIGNMENT 6 *)
    | PreDec acc -> let (loc, store1) = access acc locEnv gloEnv store (* CHANGED FOR ASSIGNMENT 6 *)
                    let res = (getSto store1 loc) - 1                  (* CHANGED FOR ASSIGNMENT 6 *)
                    (res, setSto store1 loc res)                       (* CHANGED FOR ASSIGNMENT 6 *)
